 ...
 */
export function getAcceptedForms(word: Pick<Word, 'text' | 'aliases'>): string[] {
...
export function isAnswerCorrect(guess: string, word: Pick<Word, 'text' | 'aliases'>): boolean {
 ...
 */
export function getWordPattern(text: string): number[] {
...
export function isGuessClose(
  ...
  for (const form of getAcceptedForms(word)) {
    ...
    if (distance > 0 && distance <= toleranceFor(form.length)) return true;
    ...
    if (formWords.length > 1 && guessWords.length > 0) {
      ...
      if (shared >= Math.ceil(formWords.length / 2) && shared < formWords.length) return true;
    }
  }
