import { CLIENT_EVENTS, GameStatus, SERVER_EVENTS, TIMINGS } from '@party-draw/shared';
...
export function setupSocketHandlers(io, gameManager) {
    ...
    function emitState(room) {
        if (pendingStateEmits.has(room.joinCode))
            return;
    ...
    function sendWordToDrawer(room, drawerId) {
        if (!room.currentWord)
            return;
        const payload = {
    ...
    function setupRoundTimer(room) {
        const durationMs = room.settings.roundDuration * 1000;
        room.setRoundTimer(setTimeout(() => {
    ...
    function startCountdown(room) {
        ...
        room.setCountdownTimer(setInterval(() => {
            count--;
            if (count > 0) {
                io.to(room.joinCode).emit(SERVER_EVENTS.COUNTDOWN_TICK, { count });
                return;
    ...
    function handleRoundEnd(room, reason) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on(CLIENT_EVENTS.ATTACH_TV, (data) => {
        ...
        socket.on(CLIENT_EVENTS.JOIN_GAME, (data) => {
            ...
            const isReturningPlayer = !!sessionId && Array.from(room.players.values()).some((p) => p.sessionId === sessionId);
        ...
        // ------------------------------------------------------------------
        socket.on(CLIENT_EVENTS.UPDATE_SETTINGS, (data) => {
            const room = gameManager.getRoomBySocket(socket.id);
        ...
        socket.on(CLIENT_EVENTS.SET_TEAM, (data) => {
            const room = gameManager.getRoomBySocket(socket.id);
            if (!room || !data?.teamId)
                return;
            ...
            if (!isSelf && !room.canControl(socket.id))
                return;
        ...
        // Dibujo
        // ------------------------------------------------------------------
        socket.on(CLIENT_EVENTS.SEND_STROKE_CHUNK, (data) => {
            const room = gameManager.getRoomBySocket(socket.id);
            if (!room || room.status !== GameStatus.DRAWING)
                return;
            const playerId = room.socketPlayerMap.get(socket.id);
            if (!playerId || playerId !== room.currentDrawerId)
                return;
            ...
            strokeOpen = true;
            room.appendStrokePoints(playerId, points, data.color, data.width, !!data.isEraser, isNewStroke);
            const payload = {
                points,
                color: data.color,
                width: data.width,
        ...
        socket.on('disconnect', () => {
            ...
            const allDone = responders.every((p) => room.isRiskMode ? p.hasAnswered : p.guessedCurrentRound);
            if (allDone) {
